/* ================================================================
   Hub GPSBI · GPSBI Gerencial — Navegação compartilhada
   ================================================================ */
const APP_PAGES = [
  { href:'fluxodecaixa.html', label:'💰 Fluxo de Caixa', id:'navFluxo' },
...
function appApplyTheme(t, skipSync){
  ...
  localStorage.setItem('gpsbi_theme', t);
...
function renderAppNav({ activePage, userLabel, userRole, onLogout, sb, currentUser }){
  ...
  const theme = savedInAccount || localStorage.getItem('gpsbi_theme') || 'light';
  document.documentElement.classList.toggle('light', theme==='light');

  const pages = APP_PAGES.filter(p => p.href !== 'fluxodecaixa.html' || _navCanSeeFluxo(currentUser && currentUser.email));
  ...
  if(savedInAccount) localStorage.setItem('gpsbi_theme', theme); // mantém o cache local sincronizado
